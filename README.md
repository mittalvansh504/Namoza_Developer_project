...
After the contact is successfully created or updated, the backend sends a confirmation WhatsApp message through the **WhatsApp Business API**. Once the form submission is successful, Google Tag Manager fires the **consultation_form_submitted** event, which is sent to GA4 and imported into Google Ads as a conversion. This allows Google Ads to optimize campaigns based on successful consultation form submissions.
